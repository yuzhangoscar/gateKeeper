...
test.describe.serial('GateKeeper Tests', () => {
    ...
    test('Fill in candidate info', async ({ page, uploadFile }) => {
        ...
        await page2.getByRole('button', { name: 'Continue to Additional Info' }).click();
        await page2.getByRole('combobox').selectOption('AUS');
        await page2.locator('select[name="Birth State"]').selectOption('NSW');
        await page2.getByText('Birth Town *Required').click();
        await page2.locator('form').filter({ hasText: 'National Police CheckFirst up' }).locator('input[type="text"]').fill('BirthTown');
        await page2.getByRole('textbox', { name: 'DD' }).click();
        await page2.getByRole('textbox', { name: 'DD' }).fill('11');
        await page2.getByRole('textbox', { name: 'MM' }).click();
        await page2.getByRole('textbox', { name: 'YYYY' }).click();
        await page2.getByRole('textbox', { name: 'YYYY' }).fill('1999');
        await page2.getByRole('textbox', { name: 'MM' }).click();
        await page2.getByRole('textbox', { name: 'MM' }).fill('1');
        await page2.locator('button[type="submit"]').nth(0).click();
        await page2.locator('label:text("Your Name (Name Qualification Completed In)") + div input').fill('MyName');
        await page2.locator('label:text-is("Qualification") + div input').fill('Bachelor');
        await page2.locator('label:text("Name of University / College / TAFE") + div input').fill('TAFE');
        await page2.locator('label:text("City of University / College / TAFE") + div input').fill('Sydney');
        await page2.getByRole('textbox', { name: 'Graduation Date *' }).fill('1/1/1999');
        await page2.locator('button[type="submit"]').nth(1).click();
        await page2.locator('label:text("Passport Number") + div input').fill('RS111111');
        await page2.locator('label[for="Passport Issuing Country"] ~ div > div > div').click();
        await page2.locator('div[id="menu-Passport Issuing Country"] li:text("Australia")').click();
        await page2.locator('button[type="submit"]').nth(2).click();
        await page2.locator('button:has(span:text("Confirm"))').click();
        await page2.waitForTimeout(500);
        await page2.locator('label:text("Referee Name") + div input').fill('BossOne');
        await page2.locator('label:text("Referee Email") + div input').fill('a@g.com');
        await page2.locator('label:text("Name of Company") + div input').fill('CompanyOne');
        await page2.locator('label:text("Relationship to You") + div input').fill('Boss');
        await page2.locator('label:text("From Date") + div input').fill('1/1/1999');
        await page2.locator('label:text("To Date") + div input').fill('1/1/2025');
        await page2.locator('button[type="submit"]').nth(3).click();
        await page2.getByRole('button', { name: 'Save and Continue' }).click();
        await page2.locator('button').filter({ hasText: /^Logout$/ }).click();
